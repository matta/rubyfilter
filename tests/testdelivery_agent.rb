...
class TestRFilterDeliveryAgent < TestBase
  ...
  def test_exitcode()
    ...
    assert_equal(RFilter::MTA::EXITCODE_DEFER,
		 RFilter::DeliveryAgent.exitcode(RFilter::DeliveryAgent::DeliveryPipeFailure.new("", 1)))
    assert_equal(RFilter::MTA::EXITCODE_DEFER,
  ...
  def test_deliver_pipe_error
    ...
    e = assert_exception(RFilter::DeliveryAgent::DeliveryPipeFailure) {
      lda.pipe(command, true)
    }
